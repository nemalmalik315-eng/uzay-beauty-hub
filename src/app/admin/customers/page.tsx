...
export default function CustomersPage() {
  const [tab, setTab] = useState<"all" | "reengagement">("all");
  const [customers, setCustomers] = useState<Customer[]>([]);
  const [search, setSearch] = useState("");
  const [sort, setSort] = useState("newest");
  const [showAdd, setShowAdd] = useState(false);
  const [reengagementClients, setReengagementClients] = useState<ReengagementClient[]>([]);
  const [loadingReengage, setLoadingReengage] = useState(false);
  ...
  useEffect(() => {
    if (tab === "reengagement") loadReengagement();
  }, [tab]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        <button onClick={() => setTab("all")}
          className={`px-5 py-2 rounded-lg text-sm font-medium transition-colors ${tab === "all" ? "bg-gold text-white" : "bg-white border border-gray-200 text-gray-600 hover:border-gold hover:text-gold"}`}>
          All Clients
        </button>
        <button onClick={() => setTab("reengagement")}
          className={`px-5 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2 ${tab === "reengagement" ? "bg-orange-500 text-white" : "bg-white border border-gray-200 text-gray-600 hover:border-orange-400 hover:text-orange-500"}`}>
          Re-engage
          {reengagementClients.length > 0 && tab !== "reengagement" && (
            <span className="bg-orange-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center">{reengagementClients.length}</span>
          )}
        </button>
      </div>
      ...
      {tab === "reengagement" && (
        <div className="space-y-4">
          <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
            <p className="text-sm text-orange-800 font-medium">Smart Re-engagement</p>
            <p className="text-xs text-orange-600 mt-0.5">
              Clients who visit regularly are flagged when overdue by their own rhythm. Irregular clients are flagged after 60 days.
            </p>
          </div>

          {loadingReengage ? (
            <div className="bg-white rounded-lg border border-gray-100 p-12 text-center text-gray-400 text-sm">Loading…</div>
          ) : reengagementClients.length === 0 ? (
            <div className="bg-white rounded-lg border border-gray-100 p-12 text-center">
              <p className="text-gray-400 text-sm">All clients are up to date 🎉</p>
            </div>
          ) : (
            <div className="space-y-3">
              {reengagementClients.map((c) => (
                <div key={c.id} className="bg-white rounded-lg border border-gray-100 p-4">
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex gap-3 mt-1 text-xs text-gray-400">
                        <span>Last visit: {new Date(c.lastVisit + "T00:00:00").toLocaleDateString("en-PK", { day: "numeric", month: "short" })}</span>
                        <span className="text-orange-500 font-medium">{c.daysSince} days ago</span>
                        {c.commonService && <span>Usually: {c.commonService}</span>}
                      </div>
                    </div>
                    <a
                      href={buildWaUrl(c.phone, c.message)}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex-shrink-0 flex items-center gap-1.5 bg-[#25D366] text-white text-xs font-medium px-3 py-2 rounded-lg hover:bg-[#1EBE5A] transition-colors"
                    >
                  ...
                  <details className="text-xs text-gray-400 cursor-pointer">
                    <summary className="hover:text-gray-600 select-none">Preview message</summary>
                    <p className="mt-2 bg-gray-50 rounded p-3 text-gray-600 whitespace-pre-wrap leading-relaxed">{c.message}</p>
                  </details>
                </div>
              ))}
            </div>
