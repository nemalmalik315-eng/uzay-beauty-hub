...
const DB_PATH = path.join(process.cwd(), "uzay.db");
...
function initializeDatabase(db: Database.Database) {
  ...
  // Seed stock items if none exist
  const stockCount = db.prepare("SELECT COUNT(*) as count FROM stock").get() as { count: number };
  if (stockCount.count === 0) {
    ...
    const insertStock = db.prepare(
      "INSERT INTO stock (product_name, category, quantity, min_threshold, unit_price) VALUES (?, ?, ?, ?, ?)"
    );
    for (const item of items) {
      insertStock.run(...item);
    }
  }
}
